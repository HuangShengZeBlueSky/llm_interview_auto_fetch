...
结论：应对噪声标签的核心是降低错误样本对梯度的负面影响，主要通过数据清洗、鲁棒损失函数和训练策略优化实现。
关键点：
1. 数据层面：利用交叉验证或置信学习（如Cleanlab）识别并剔除或纠正高损耗样本。
2. 损失函数：采用对噪声鲁棒的损失（如MAE、Symmetric Cross Entropy），或动态调整样本权重（如基于预测一致性降权）。
3. 训练策略：利用神经网络先拟合简单样本的特性进行早停；或采用半监督学习，将高疑样本视为无标签数据进行伪标签训练。
...
结论：KF中包含过程噪声矩阵(Q)和测量噪声矩阵(R)。预测（运动）步估计噪声变大，更新（修正）步估计噪声变小。
关键点：
1. 矩阵含义：Q代表系统动力学模型的不确定性（如未建模的干扰），R代表传感器测量的不确定性（如硬件误差）。
2. 运动方程：引入了新的过程噪声Q，且状态转移会放大原有误差，因此先验估计的协方差（估计噪声）变大。
3. 修正方程：融合了新的观测信息，通过卡尔曼增益对先验进行加权修正，系统信息量增加，不确定性降低，因此后验估计的协方差变小。
...
结论：幻觉本质是模型在压缩训练数据时产生的概率分布偏差，导致生成看似合理但违背事实的内容。
关键点：
1. 数据层面：提升预训练和SFT数据质量，剔除冲突文本，增强事实性约束。
2. 训练层面：采用RLHF/DPO对齐人类偏好，惩罚过度自信的错误输出，或引入事实一致性损失。
3. 推理层面：首选RAG（检索增强生成）引入外部可靠知识库作为上下文锚点；采用CoT（思维链）降低复杂推理错误率；通过Self-Consistency（自洽性）或多Agent辩论进行结果校验与纠错。

## 37. SFT模型经常在上下文比较长或者调查轮次多了之后就会出现查询工具的重复调用现象，如何消除这种现象？

结论：重复调用本质是长上下文中模型注意力衰减及状态追踪能力不足。需从数据构造、Prompt设计和推理干预三方面解决。
关键点：
1. 数据构造：在SFT数据中增加长轮次、多工具交互的负样本（如明确拒绝重复调用的对话），强化模型对“已获取信息”的状态感知。
2. Prompt设计：在System Prompt中强制要求模型先输出当前状态摘要（State Tracking）或已调用工具列表，再决定下一步动作。
3. 推理干预：修改解码逻辑，若检测到连续生成相同工具调用指令，则施加极大的Logit惩罚（Repetition Penalty），或强制截断并插入提示引导其生成最终回复。
