...
> 图示见原始资料：assert/arcface.png#pic_center=600x400
...
> 图示见原始资料：assert/centernet.jpg#pic_center=600x400
...
> 图示见原始资料：assert/cbam.png#pic_center=600x400
...
> 图示见原始资料：assert/foucs.jpg#pic_center=600x400
...
> 图示见原始资料：assert/detr.png#pic_center=600x400
...
结论：ORB特征提取速度极快，但存在尺度不变性较弱、对光照/视角变化敏感以及特征点分布易扎堆三大缺陷。
关键点：
1. 尺度不变性弱：仅依赖图像金字塔实现离散尺度不变。改进：构建更精细的多尺度金字塔，或引入连续尺度空间，但会牺牲速度。
2. 特征点分布不均：FAST角点易在局部纹理区聚集。改进：采用ORB-SLAM中的四叉树（Quad-tree）或网格划分策略，强制特征点均匀分布，提升位姿估计精度。
3. 光照与视角敏感：BRIEF描述子对强光照和仿射变换鲁棒性差。改进：提取前进行直方图均衡化等光照补偿；或在现代感知任务中，直接采用SuperPoint等深度学习特征替代，大幅提升极端环境下的匹配鲁棒性。
