...
> 图示见原始资料：assert/focal.jpg#pic_center
...
> 图示见原始资料：assert/bn.png#pic_center
...
BEVFormer多视角输入处理的核心是将6张环视图像提取特征并展平，结合相机内外参生成Spatial Cross-Attention所需的Reference Points。
关键实现步骤：
1. 维度转换：输入为`(B, 6, 3, H, W)`，需合并Batch和View维度为`(B*6, 3, H, W)`送入Backbone提取多尺度特征。
2. 特征重构：提取特征后，恢复维度为`(B, 6, C, H', W')`，并展平空间维度为`(B, 6, H'*W', C)`，作为Transformer的Value。
3. 编码融合：需叠加2D图像位置编码和Camera-level视角编码，以保留空间与视角先验。
4. 投影采样：利用相机内外参，将BEV空间的3D网格投影到6个2D视图，生成归一化的参考点用于Deformable Attention。

## 38. todo

此为占位符题目。在大厂手撕代码面试中，遇到未明确或开放性问题，核心是主动与面试官沟通澄清需求，切忌盲目动笔。
关键应对策略：
1. 澄清题意：明确输入输出格式、数据规模、时间空间复杂度要求及边界条件。
2. 沟通思路：先给出基础解法，再推导最优解法，双方达成共识后再开始编码。
3. 代码规范：保持良好的变量命名习惯，注重模块化设计，务必包含异常处理（如空指针、数组越界等）。
4. 主动验证：代码完成后，主动构造常规测试用例和Corner Case（如空数组、极值）进行Dry Run，展现严谨的工程素养。
