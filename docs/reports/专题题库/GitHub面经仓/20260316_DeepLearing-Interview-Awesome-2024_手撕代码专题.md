# GitHub 提取：DeepLearing-Interview-Awesome-2024_手撕代码专题

> 来源仓库：[DeepLearing-Interview-Awesome-2024](https://github.com/315386775/DeepLearing-Interview-Awesome-2024/blob/main/CodeAnything/Reference.md)
> 本页共整理 40 道题，尽量保留原仓库答案；若原仓库缺少解析，则自动补齐。
...
> 图示见原仓库资源：assert/focal.jpg#pic_center
...
> 图示见原仓库资源：assert/bn.png#pic_center
...
## 22. Numpy 实现线性回归损失函数，输入直线对应的坐标点，输出损失？
...
## 25. Numpy 实现线性回归，输入学习率、迭代次数及坐标点
...
## 30. Pytorch 使用torch.utils.data.Dataset类来构建自定义的数据集，根据文件名后缀来创建一个图像分类的数据集
...
## 33. 现在可以使用PyTorch的DataLoader来加载数据集
...
## 34. PyTorch 构建一个自定义层，该层实现一个简单的LReLU激活函数。
...
## 36. PyTorch 实现图像到Patch Embedding过程，提示可用卷积实现？
...
## 37. PyTorch 代码实现 BEVFormer 的六张图输入部分

结论：BEVFormer的六视角输入处理核心是将`(B, 6, C, H, W)`的图像展平后通过Backbone提取多尺度特征，再还原维度供后续Spatial Cross-Attention使用。
关键点：
1. **维度变换**：输入张量包含Batch、6个视角（前、后及四个侧边）、通道、高、宽。需先用`view(B*6, C, H, W)`合并B和N维度。
2. **特征提取**：送入ResNet等Backbone提取多尺度特征（如C2到C5）。
3. **维度还原**：将提取的特征通过`view(B, 6, C', H', W')`还原，以便结合相机内外参生成3D参考点。
4. **核心代码**：
```python
B, N, C, H, W = x.shape
x = x.view(B * N, C, H, W)
mlvl_feats = self.backbone(x) # 提取多尺度特征
mlvl_feats = [feat.view(B, N, *feat.shape[1:]) for feat in mlvl_feats]
```

## 38. todo

结论：此为占位符题目。在真实大厂面试中，手撕代码环节通常会考察大模型或深度学习底层核心算子的 PyTorch 实现。
关键点：
1. **沟通确认**：遇到不明确的题目，首要是与面试官沟通，明确输入输出维度及具体算法要求。
2. **高频考点**：大模型方向常考 Multi-Head Self-Attention (MHSA)、RoPE (旋转位置编码)、KV Cache 机制、LayerNorm 或 Beam Search 等。
3. **编码规范**：手撕代码时需注意张量维度的变换（如 `transpose` 和 `contiguous` 的使用）、计算效率（如避免显式 for 循环，多用矩阵乘法或 `torch.einsum`），以及边界条件和数值稳定性（如 Softmax 前的缩放）的处理。

## 39. 使用自定义的PatchEmbedding层

```

## 40. C++中与类型转换相关的4个关键字特点及应用场合
