# GitHub 提取：DeepLearing-Interview-Awesome-2024_行业算法专题

> 来源仓库：[DeepLearing-Interview-Awesome-2024](https://github.com/315386775/DeepLearing-Interview-Awesome-2024/blob/main/IndustryAlgorithm/Reference.md)
> 本页共整理 37 道题，尽量保留原仓库答案；若原仓库缺少解析，则自动补齐。
...
结论：处理噪声标签的核心在于降低错误样本对梯度更新的负面影响，主要通过数据清洗、鲁棒损失函数和训练策略优化来实现。
关键点：
1. 数据清洗：利用交叉验证或置信学习（如Cleanlab）识别并剔除高损失或低置信度样本。
2. 鲁棒损失：使用对噪声不敏感的损失函数（如MAE、Focal Loss变体）或标签平滑（Label Smoothing）防止模型过拟合错误标签。
3. 训练策略：采用早停法（模型通常先拟合简单正确样本，后拟合噪声）；或使用协同教学（Co-teaching），双网络互相挑选低损失样本更新。
4. 半监督学习：将疑似错误标签的样本剥离标签，视为无监督数据，利用伪标签或一致性正则化进行训练。
...
结论：卡尔曼滤波（KF）中的噪声矩阵用于量化系统的不确定性。运动方程（预测步）会使估计噪声变大，而修正方程（更新步）会使估计噪声变小。
关键点：
1. 矩阵含义：过程噪声协方差矩阵（Q）表示状态转移模型的不准确性或外部未建模干扰；测量噪声协方差矩阵（R）表示传感器观测的固有误差。
2. 运动方程（预测）：根据上一时刻状态预测当前状态，由于引入了过程噪声Q，系统的不确定性发生累积，导致先验估计误差协方差矩阵（估计噪声）变大。
3. 修正方程（更新）：结合实际观测值对预测进行校正，融合了预测与观测多源信息，卡尔曼增益的最优分配使得后验估计误差协方差矩阵收敛变小。
...
结论：大模型幻觉的本质是模型在缺乏真实知识或上下文时，基于概率分布强行生成看似合理但事实错误的文本。
关键点：
1. 检索增强生成（RAG）：引入外部知识库，在生成前检索相关事实作为Prompt上下文，限制模型自由发挥，是目前工业界最有效的方案。
2. 训练对齐优化：在SFT和RLHF阶段，增加拒答机制（如“我不知道”），惩罚无根据的生成，提升模型的事实性边界意识。
3. 数据层面：预训练数据存在噪声或知识截断，需通过高质量数据清洗和持续预训练缓解。
4. 推理干预：采用多智能体辩论、自我一致性（Self-Consistency）或调整解码策略（如降低Temperature）来减少随机性错误。

## 37. SFT模型经常在上下文比较长或者调查轮次多了之后就会出现查询工具的重复调用现象，如何消除这种现象？

结论：长上下文导致工具重复调用的本质是模型注意力衰减和状态追踪能力不足。可通过强化状态记忆、优化SFT数据及推理干预来消除。
关键点：
1. 优化SFT数据分布：在训练集中增加长轮次、多工具调用的负样本（如重复调用被拒绝的轨迹），教会模型识别已执行的操作并推进任务。
