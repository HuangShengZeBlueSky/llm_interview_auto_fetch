...
> 图示见原始资料：assert/sd1.jpg#pic_center
...
> 图示见原始资料：assert/attention.png#pic_center=600x400
...
> 图示见原始资料：assert/Q.png#pic_center
...
> 图示见原始资料：assert/infer.png#pic_center
...
Qwen2.5-VL通过**动态分块**与**M-RoPE**实现了对任意分辨率和宽高比的原生支持。
关键点：
1. **动态分块**：摒弃传统强制Resize，根据图像原始宽高比动态划分Patch，生成变长视觉Token序列，避免形变与细节丢失。
2. **M-RoPE空间编码**：为处理变长序列，将1D-RoPE扩展为2D，分别对高度和宽度方向的绝对位置进行编码，精准保留二维空间结构。
3. **视觉池化**：ViT处理后通过池化层压缩Token数量，维持空间特征的同时大幅降低LLM计算复杂度。
