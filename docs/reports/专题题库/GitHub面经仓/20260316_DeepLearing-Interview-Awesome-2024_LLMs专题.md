# GitHub 提取：DeepLearing-Interview-Awesome-2024_LLMs专题

> 来源仓库：[DeepLearing-Interview-Awesome-2024](https://github.com/315386775/DeepLearing-Interview-Awesome-2024/blob/main/LLMs/Reference.md)
> 本页共整理 60 道题，尽量保留原仓库答案；若原仓库缺少解析，则自动补齐。
...
> 图示见原仓库资源：assert/sd1.jpg#pic_center
...
> 图示见原仓库资源：assert/attention.png#pic_center=600x400
...
> 图示见原仓库资源：assert/Q.png#pic_center
...
> 图示见原仓库资源：assert/infer.png#pic_center
...
对于多模态理解，使用 SigLIP Encoder 从图像中提取高维语义特征 (和 Janus 一致)。
对于视觉生成任务，使用 LLamaGen中的 VQ tokenizer 将图像转换为离散的 ID (和 Janus 一致)。
...
核心思想是将词嵌入向量的某些维度视为复数，然后通过旋转矩阵来编码位置信息。

https://www.zhihu.com/question/1821771428/answer/1961032470133737195

## 60. Qwen2.5-VL的原生分辨率具体是如何做的？

结论：Qwen2.5-VL 依靠“动态 Patch 划分”与“多模态旋转位置编码(mRoPE)”实现原生分辨率支持，摒弃了传统固定分辨率的缩放与裁剪。

关键点：
1. 动态 Patch 划分：保持图像原始宽高比，按固定大小动态切分图像。不同分辨率产生不同长度的视觉 Token 序列，避免图像形变与信息丢失。
2. mRoPE (2D-RoPE)：为解决动态尺寸的位置编码问题，将 1D RoPE 扩展为 2D。对图像高、宽独立进行旋转位置编码，精准表征任意宽高比的空间关系。
3. Token 压缩：高分辨率会产生超长序列，模型在 ViT 后接入池化层（如 2x2 降采样）压缩视觉 Token 数量，有效控制 LLM 的计算开销。
